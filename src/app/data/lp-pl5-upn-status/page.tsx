...
import { listLoadedLpPl5UpnStatusRows } from "@/server/repositories";
...
export default async function LpPl5UpnStatusPage() {
  const rows = await listLoadedLpPl5UpnStatusRows();
  ...
  return (
    <DataPageLayout
      title="LP-产品分类-SKU状态表"
      description="过程数据表。根据基础表中的可观察 LP-产品分类-SKU 关系与状态实时派生到 SKU 明细层，不需要先执行结果计算。"
      statsItems={[
        { label: "最近月份", value: latestPeriod ?? "-" },
        { label: "最近月份明细数", value: latestRows.length },
      ...
      table={{
        ...
        exportTable: "list_lp_pl5_upn_status_monthly",
        exportParams: {},
        filters: [
          {
            key: "period_month",
