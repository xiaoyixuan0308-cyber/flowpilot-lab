...
import { listLoadedFcstT2Pl5Rows } from "@/server/repositories";

type Row = { id: string; parentdealerlpcode: string | null; parentdealerlpname: string | null; sc_bu: string; pl5_code: string; pl5_name: string | null; fcst_qty: string; year: string; month: string };

const columns: Column<Row>[] = [
  { key: "parentdealerlpcode", header: "上级经销商" },
  { key: "parentdealerlpname", header: "上级经销商名" },
...
export default async function FcstT2Pl5Page() {
  const rows = await listLoadedFcstT2Pl5Rows();
  const data: Row[] = rows.map((r) => ({
    ...
    year: r.year, month: r.month, fcst_qty: r.fcst_qty.toString(),
  }));
  ...
  return (
    <DataPageLayout
      title="T2-产品分类预测表"
      description="输入基础表。提供 V，对应 T2-产品分类 层预测。"
      actions={<BaseTableActions table="fcstT2Pl5" />}
      statsItems={[
      ...
      table={{
        ...
        exportTable: "ods_fcst_t2_pl5_monthly",
        exportParams: {},
      }}
    />
  );
